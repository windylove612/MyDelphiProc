...
function inflate_blocks_free(s : pInflate_blocks_state;
                             var z : z_stream) : int_t;

procedure inflate_set_dictionary(var s : inflate_blocks_state;
                                 const d : array of byte;  { dictionary }
                                 n : uInt);         { dictionary length }

function inflate_blocks_sync_point(var s : inflate_blocks_state) : int_t;

implementation

uses
  QImport3infcodes, QImport3inftrees, QImport3infutil;
...
function inflate_blocks (var s : inflate_blocks_state;
                         var z : z_stream;
                         r : int_t) : int_t;           { initial return code }
label
        ...
        begin
          ...
          if (ptr2int(q) >= ptr2int(s.read)) then
            Tracev('inflate:       stored end '+
                IntToStr(uInt(z.total_out) + ptr2int(q) - ptr2int(s.read)) + ' total out')
          else
            Tracev('inflate:       stored end '+
                    IntToStr(uInt(z.total_out) + ptr2int(s.zend) - ptr2int(s.read) +
...
function inflate_blocks_sync_point(var s : inflate_blocks_state) : int_t;
begin
  inflate_blocks_sync_point := int_t(s.mode = LENS);
end;
